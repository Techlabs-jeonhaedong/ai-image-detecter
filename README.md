...
API 키·과금·인터넷 연결 없이 로컬에서 추론한다. (첫 실행 시 모델 1회 다운로드 후 오프라인 동작)
...
첫 실행 때 모델(~300MB)이 `~/.cache/huggingface/` 에 자동 다운로드된다. 이후 실행은 캐시를 사용해 오프라인으로 동작한다.
...
| 환경변수 | 기본값 | 설명 |
|----------|--------|------|
| `DETECTOR_BACKEND` | `torch` | 추론 백엔드: `torch` 또는 `onnx` |
| `ONNX_MODELS_DIR` | `onnx_models` | ONNX 모델 디렉토리 (`DETECTOR_BACKEND=onnx` 시 사용) |
